...
function add(id){
	var addminus = $('.addminus_'+id+'').val();
	var given = $('#newStocks_'+id+'').val();
	var add = parseInt(addminus) + parseInt(given);

	if(addminus == '')
	{
		alert('Input text must hava a value !');
		$('.addminus_'+id+'').focus();
		return false;
	}
	else
	{
		var location_a = 'http://localhost/thesis/main_controller/add_stocks/'+id;
		...
		$.ajax({
			...
			dataType:"json",
			success: function(){
				var mylink = 'http://localhost/thesis/main_controller/add_get_new_stocks/'+id;
				$.getJSON(mylink, function(data){
				$.each(data, function(key, val){
							$('#newStocks_'+id+'').val(val.product_serial);
							$('.addminus_'+id+'').val('');
						});
					});
				}
...
function minus(id){
	var addminus = $('.addminus_'+id+'').val();
	var given = $('#newStocks_'+id+'').val();
	var minus = parseInt(addminus) - parseInt(given);
	...
	{
		var location_m = 'http://localhost/thesis/main_controller/minus_stocks/'+id;
...
<table id="result_table" style="margin-left: 770px;">
	<tr>
...
<div id="result">
		...
		echo  '<tr>';
		echo  '<td class="text">Stocks</td><td class="infotxt">'.form_input($newStocks).'&nbsp;<code id="edits">&nbsp;edit</code><input type="text" name="add" onKeyup="identifier(this.value)" id="addminus" class="addminus_'.$row->product_id.'">&nbsp;<input type="submit" onclick="javascript: add('.$row->product_id.')" name="add" id="add" value="+"><input onclick="javascript: minus('.$row->product_id.')" type="submit" name="minus" id="minus" value="-">&nbsp;&nbsp;<code id="cans">Done</code></td>';
		echo  '</tr>';
